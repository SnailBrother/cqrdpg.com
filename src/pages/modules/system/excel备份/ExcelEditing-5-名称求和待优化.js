...
const saveCellEditToSheet = (sheetName, cellAddress, value) => {
  ...
  // ✅ 触发依赖重算
  const currentSheetNamedRanges = namedRanges.filter(range => range.sheet === sheetName);
  console.log('当前工作表命名区域:', currentSheetNamedRanges);
  
  const graph = dependencyGraphRef.current[sheetName];
  console.log('依赖图:', graph);
  
  if (graph) {
    console.log(`重新计算依赖 ${cellAddress} 的单元格`);
    recalculateDependents(cellAddress, ws, graph, currentSheetNamedRanges);
  } else {
    console.warn(`工作表 ${sheetName} 的依赖图未找到`);
  }
};
...
const saveCellEdit = (cellAddress, value) => {
  ...
  console.log(`保存单元格 ${cellAddress} = ${value}`);
  console.log('当前命名区域:', namedRanges);
  ...
  if (namedRange) {
    console.log(`更新命名区域 ${namedRange.name} = ${value}`);
    setFormValues(prev => ({ ...prev, [namedRange.name]: value }));
    
    // 更新命名区域数组中的值
    const updatedNamedRanges = namedRanges.map(range => 
      range.name === namedRange.name ? { ...range, value } : range
    );
    setNamedRanges(updatedNamedRanges);
  }
