...
-- 1. 先创建目标表（修正语法错误后的版本）
-- ============================================
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'WeChatThemeSettings' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
    CREATE TABLE WeChatApp.dbo.WeChatThemeSettings (
        ...
        their_font_color NVARCHAR(7) DEFAULT '#000000',
        their_bubble_color NVARCHAR(7) DEFAULT '#D3D3D3',
        my_font_color NVARCHAR(7) DEFAULT '#000000',
        my_bubble_color NVARCHAR(7) DEFAULT '#90EE90',
        background_color NVARCHAR(7) DEFAULT '#F0F0F0',
        use_background_image BIT DEFAULT 0,
        navbar_font_color NVARCHAR(7) DEFAULT '#FFFFFF',
        navbar_background_color NVARCHAR(7) DEFAULT '#32502c'
    );
    PRINT '目标表 WeChatApp.dbo.WeChatThemeSettings 创建成功';
END
ELSE
BEGIN
    PRINT '目标表已存在，将直接进行数据复制';
END
GO
...
SELECT @SourceCount = COUNT(*) FROM BillingApp.dbo.AccountLogin;
PRINT '源表 BillingApp.dbo.AccountLogin 共有 ' + CAST(@SourceCount AS VARCHAR) + ' 条记录';
GO

-- ============================================
-- 3. 开启 IDENTITY_INSERT，允许显式插入主键值
-- ============================================
SET IDENTITY_INSERT WeChatApp.dbo.WeChatThemeSettings ON;
GO

-- ============================================
-- 4. 复制数据，保留原有主键
-- ============================================
INSERT INTO WeChatApp.dbo.WeChatThemeSettings (
    id,
    username,
    password,
    login_time,
...
SELECT 
    ...
    navbar_font_color,
    navbar_background_color
FROM BillingApp.dbo.AccountLogin;
GO

-- 显示复制了多少条记录
PRINT '数据复制完成，共复制 ' + CAST(@@ROWCOUNT AS VARCHAR) + ' 条记录';
GO

-- ============================================
-- 5. 关闭 IDENTITY_INSERT，恢复自增特性
-- ============================================
SET IDENTITY_INSERT WeChatApp.dbo.WeChatThemeSettings OFF;
GO

-- ============================================
-- 6. 重置自增种子，确保后续新增记录从正确的主键值开始
-- ============================================
DECLARE @MaxID INT;
SELECT @MaxID = ISNULL(MAX(id), 0) FROM WeChatApp.dbo.WeChatThemeSettings;
DBCC CHECKIDENT ('WeChatApp.dbo.WeChatThemeSettings', RESEED, @MaxID);
PRINT '自增种子已重置为: ' + CAST(@MaxID AS VARCHAR);
GO
...
-- ============================================
SELECT 
    '源表 (BillingApp.dbo.AccountLogin)' AS TableName,
...
UNION ALL
SELECT 
    '目标表 (WeChatApp.dbo.WeChatThemeSettings)' AS TableName,
    COUNT(*) AS RecordCount,
    MIN(id) AS MinID,
    MAX(id) AS MaxID
...
SELECT TOP 5 
    id, username, status, 
    their_font_color, their_bubble_color,
    my_font_color, my_bubble_color
FROM BillingApp.dbo.AccountLogin 
ORDER BY id;
