...
import "./UploadBuildingsPricePicture.css";
import { Loading } from '../../../../components/UI';
import WordReportGeneratorLoader from '../../accounting/Notification/WordReportGeneratorLoader';

const UploadBuildingsPricePicture = () => {
  // 修改页面标题
  useEffect(() => {
    document.title = '建筑物照片上传';
  }, []);
  
  // 使用useSearchParams获取查询参数
  const [searchParams] = useSearchParams();
  // 从查询参数中获取buildingsPriceid和建筑信息
  ...
  const fetchExistingImages = async () => {
    try {
      ...
      setMessage("获取现有图片失败：" + error.message);
    }
  };
  ...
  const handleFileChange = (e) => {
    ...
    for (const file of selectedFiles) {
      if (!file.type.match(/image\/jpeg/) && !file.type.match(/image\/jpg/) && !file.type.match(/image\/png/) && !file.type.match(/image\/gif/)) {
        setMessage("只支持 .jpg, .jpeg, .png, .gif 格式的图片");
        return;
      }
  ...
  const handleRemoveFile = (index) => {
    if (compressionMode) {
      ...
      const removedFile = newCompressedFiles[index];
      // 释放对象URL
      if (removedFile.url) {
  ...
  const handleUpload = async () => {
    ...
    formData.append("buildingsPriceid", buildingsPriceid);
    ...
    for (const fileData of filesToUpload) {
      if (compressionMode) {
        ...
        const fileName = `${fileData.name.split('.')[0]}_compressed.${extension}`;
        formData.append("images", fileData.blob, fileName);
      } else {
    ...
    try {
      const response = await axios.post("/api/UploadBuildingsPricePicture", formData, {
        headers: {
  ...
  return (
    <div className="uphpPicture-container">
      ...
      <div className="uphpPicture-header">
        <h2>{name || "未命名建筑"} - 图片上传</h2>
        <div className="uphpPicture-building-info">
          <p><strong>结构：</strong>{structure || "未设置"}</p>
          <p><strong>区域：</strong>{area || "未设置"}</p>
          <p><strong>价格：</strong>{price ? `${price} ${unit}` : "未设置"}</p>
          <p><strong>数据ID：</strong>{buildingsPriceid}</p>
        </div>
        
        <p className="uphpPicture-instructions">
          {compressionMode ? 
            "压缩完成，请确认压缩后的图片效果" : 
            "请上传相关图片 (支持JPG/PNG/GIF格式，最大10MB)"
          }
        </p>
        ...
        {existingImages.length > 0 && (
          <p className="uphpPicture-existing-info">
            当前建筑已有 {existingImages.length} 张图片
          </p>
        )}
      </div>
      ...
      >
        {displayFiles.length === 0 ? (
          <label htmlFor="file-upload" className="uphpPicture-prompt">
            ...
            <p>点击或拖拽文件到此处上传</p>
            <p className="uphpPicture-hint">支持JPG/PNG/GIF格式，最大10MB，最多30张</p>
            {existingImages.length > 0 && (
                ...
                <div key={index} className="uphpPicture-preview-item">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.target.src = "data:image/svg+xml;base64,";
                    }}
                  />
                  ...
                  <div className="uphpPicture-file-info">
                    <span className="uphpPicture-file-name" title={compressionMode ? file.name : file.name}>
                      {compressionMode ? file.name : file.name}
                    </span>
                    <span className="uphpPicture-file-size">
                      ...
                      {compressionMode && file.originalType && ` (${file.originalType.split('/')[1]})`}
                    </span>
                  </div>
                  {compressionMode && (
      ...
      {message && (
        <div className={`uphpPicture-message ${message.includes('失败') || message.includes('错误') || message.includes('超过') || message.includes('只支持') ? 'error' : 'success'}`}>
          {message}
        </div>
      )}
...
export default UploadBuildingsPricePicture;