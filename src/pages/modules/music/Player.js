...
const Player = ({ className = '' }) => {
  ...
  // 处理音量轨道点击（直接点击进度条设置音量）
  const handleVolumeTrackClick = (e) => {
    e.preventDefault();
  ...
  const handleSongEnd = () => {
    if (currentSong && progress > duration * 0.5) {
      increasePlayCount(currentSong);
    }
