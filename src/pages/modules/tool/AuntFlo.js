...
const AuntFlo = () => {
  ...
  const fetchPeriodRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/auntflo/records?username=${encodeURIComponent(username)}&startDate=${formatDate(startDate)}&endDate=${formatDate(endDate)}`
      );
  ...
  const saveRecord = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('/api/auntflo/records', {
        method: 'POST',
        headers: {
