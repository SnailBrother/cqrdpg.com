...
const ExcelEditing = () => {
  ...
  // 其他 useCallback / useMemo 函数（保持不变）
  ...
  const exportExcel = useCallback(async () => {
    ...
    try {
      ...
      workbook.SheetNames.forEach(sheetName => {
        ...
        const newWs = newWorkbook.addWorksheet(sheetName);
        newWs.columns = Array.from({ length: 26 }, (_, i) => ({ width: 12 }));

        for (let R = 0; R < 150; R++) {
          for (let C = 0; C < 26; C++) {
            const cellRef = XLSX.utils.encode_cell({ r: R, c: C });
            const oldCell = oldWs[cellRef];
            const newCell = newWs.getCell(R + 1, C + 1);

            if (oldCell) {
              if (oldCell.f) {
                newCell.value = { formula: oldCell.f, result: oldCell.v ?? '' };
              } else {
                if (oldCell.t === 'n' || oldCell.t === 'b') {
                  newCell.value = oldCell.v ?? '';
                } else if (oldCell.t === 'd') {
                  newCell.value = new Date(oldCell.v);
                } else {
                  newCell.value = oldCell.v ?? '';
                }
              }
              ...
              if (s.fill?.fgColor?.rgb) {
                style.fill = {
                  type: 'pattern',
                  pattern: 'solid',
                  fgColor: { argb: s.fill.fgColor.rgb }
                };
              }
              ...
              // ✅ 强制 A1:E22 边框
              const shouldHaveBorder = R >= 0 && R <= 21 && C >= 0 && C <= 4;
              if (shouldHaveBorder) {
                style.border = {
              ...
              newCell.style = { font: { color: { argb: 'FF000000' } } };
            }
          }
        }
      });
  ...
  // ✅ 包裹整个组件以使用 ConfirmationDialogManager（虽然当前未用 confirm，但保留结构）
  return (
