...
const exportExcel = useCallback(async () => {
  ...
  try {
    ...
    namedRanges.forEach(range => {
      ...
      const value = formValues[range.name];
      let finalValue = value;

      // 尝试转为数字（如果合适）
      const numVal = parseFloat(value);
      if (!isNaN(numVal) && value !== '' && !value.includes(',')) {
        finalValue = numVal;
      } else if (value === '') {
        finalValue = null; // 清空
      }
      ...
      // 写入单元格（仅值，不破坏其他属性）
      sheet.getCell(range.address).value = finalValue;
    });

    // Step 4: （可选）增强样式，如加边框（按需保留或删除）
