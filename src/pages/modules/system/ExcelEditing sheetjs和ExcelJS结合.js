...
//其他没得什么问题，但是导出文件的时候会报错，但是不影响正常操作 可以留着备用
...
const exportExcel = useCallback(async () => {
  ...
  try {
    ...
    // Step 1: 用 SheetJS 将当前 workbook（含命名区域）写入 ArrayBuffer
    const sheetjsBuffer = XLSX.write(workbook, {
      type: 'array',
      bookType: 'xlsx',
      cellStyles: true,
      cellFormula: true
    ...
    // Step 2: 用 ExcelJS 读取这个 buffer，自动继承命名区域等元数据
    const exceljsWorkbook = new ExcelJS.Workbook();
    await exceljsWorkbook.xlsx.load(sheetjsBuffer);

    // Step 3: 遍历每个工作表，仅增强样式（例如 A1:E22 加边框）
    exceljsWorkbook.eachSheet((worksheet) => {
      // 设置列宽（可选）
      worksheet.columns = Array.from({ length: 26 }, (_, i) => ({ width: 12 }));

      // 为 A1:E22（即行 1-22，列 A-E）添加边框
      const startRow = 1;
      const endRow = 22;
      const startCol = 1;
      const endCol = 5;
