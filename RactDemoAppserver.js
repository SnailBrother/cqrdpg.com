...
app.get('/api/getrealestatepicturecarouselimages', (req, res) => {
    ...
    fs.readdir(imageDir, (err, files) => {
        ...
        const imageUrls = imageFiles.map(file =>
            `http://www.cqrdpg.com/backend/images/Community/${encodeURIComponent(region)}/${encodeURIComponent(folder)}/${file}`
        );
...
{ //好友聊天//聊天👇
    ...
    app.post('/api/messages/uploadImage',
        ...
        async (req, res) => {
            try {
                ...
                // 构建图片访问URL
                const imageUrl = `http://www.cqrdpg.com/backend/images/ChatImages/${sender_name}/${req.file.filename}`;
    ...
    app.get('/api/getuserheadimage', (req, res) => {
        ...
        fs.readdir(imageDir, (err, files) => {
            ...
            // 返回第一张图片的URL
            const imageUrl = `http://www.cqrdpg.com/backend/images/ChatApp/${encodeURIComponent(username)}/headpicture/${imageFiles[0]}`;
            res.json({ imageUrl });
        });
    });
    ...
    app.post('/backend/api/uploaduserheadimage', uploadheadimage.single('image'), (req, res) => {
        ...
        // 返回新的头像 URL
        const imageUrl = `http://www.cqrdpg.com/backend/images/ChatApp/${encodeURIComponent(username)}/headpicture/${req.file.filename}`;
        res.json({ imageUrl });
    });
    ...
    app.post('/backend/api/uploadchatbackground', uploadChatBackground.single('image'), (req, res) => {
        ...
        // 返回新的背景图片 URL
        const imageUrl = `http://www.cqrdpg.com/backend/images/ChatApp/${encodeURIComponent(username)}/chatbackgroundimage/${req.file.filename}`;
        res.json({ imageUrl });
    });
    ...
    try {
        ...
        const response = {
            ...
            images: imagesResult.recordset.map(img => ({
                ...img,
                url: `http://www.cqrdpg.com/backend/images/DressingGuidelinesData/${formattedDate}/${img.src}`
            }))
        };
...
app.get('/api/dressing-guidelines/today', async (req, res) => {
    try {
        ...
        const response = {
            ...
            images: imagesResult.recordset.map(img => ({
                ...img,
                url: `http://www.cqrdpg.com/backend/images/DressingGuidelinesData/${today}/${img.src}`
            }))
        };
        ...
        const resultsWithImages = await Promise.all(
            weatherResults.recordset.map(async (weatherItem) => {
                ...
                return {
                    ...
                    images: imagesResult.recordset.map(img => ({
                        ...img,
                        url: `http://www.cqrdpg.com/backend/images/DressingGuidelinesData/${weatherItem.date}/${img.src}`
                    }))
                };
            })
        );
    ...
    async (req, res) => {
        try {
            ...
            for (let i = 0; i < req.files.length; i++) {
                ...
                const imageUrl = `http://www.cqrdpg.com/backend/images/DressingGuidelinesData/${date}/${file.originalname}`;
            ...
            res.json({
                ...
                images: req.files.map(file => ({
                    ...
                    path: `http://www.cqrdpg.com/backend/images/DressingGuidelinesData/${date}/${file.originalname}`
                }))
            });
...
app.get('/api/getWardrobeStewardData', async (req, res) => {
    try {
        ...
        result.recordset.forEach(item => {
            // 构建两张图片URL
            const itemImageUrl = `http://www.cqrdpg.com/backend/images/WardrobeStewar/${item.category}/${item.item_code}.png`;
            const effectImageUrl = `http://www.cqrdpg.com/backend/images/WardrobeStewar/${item.category}/${item.item_code}effect.png`;
        ...
        try {
            ...
            res.json({
                ...
                images: newFiles.map(file => ({
                    filename: file.originalname,
                    path: `http://www.cqrdpg.com/backend/images/HousePricePictures/${reportsID}/${file.originalname}`
                }))
            });
        ...
        const dataQuery = `
            SELECT *,
                CASE 
                    ...
                    THEN CONCAT('http://www.cqrdpg.com/backend/musics/', coverimage)
                    ELSE 'http://www.cqrdpg.com/backend/musics/default.jpg'
                END as coverimage_url,
                CONCAT('http://www.cqrdpg.com/backend/musics/', src) as src_url
    ...
    try {
      ...
      `);
        ...
        const insertResult = await pool.request()
            ...
            .input('coverimage', sql.NVarChar, 'http://www.cqrdpg.com/backend/musics/default.jpg')
            ...
            `;
                ...
                res.json({
                    ...
                    images: newFiles.map(file => ({
                        ...
                        buildingsPriceid: buildingsPriceid,
                        url: `http://www.cqrdpg.com/backend/images/BuildingsPricePictures/${buildingsPriceid}/${file.originalname}`
                    }))
                });
...
ORDER BY 
    ...
    app.post('/api/mergePdfs', async (req, res) => {
        try {
            ...
            const url = `http://www.cqrdpg.com/backend/mergedpdf/${filename}`;
