...
function Equipment() {
  ...
  const fetchEquipmentData = async () => {
    try {
      // 发送 GET 请求获取设备数据
      const response = await axios.get("/api/getMachineryEquipmentPricesTable");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedEquipment) {
        // 更新设备
        await axios.put(`/api/updateMachineryEquipmentPricesTable/${selectedEquipment.id}`, newEquipment);
      } else {
        // 添加新设备
        await axios.post("/api/addMachineryEquipmentPricesTable", newEquipment);
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`api/deleteMachineryEquipmentPricesTable/${id}`);
      fetchEquipmentData(); // 刷新设备列表
    } catch (error) {
