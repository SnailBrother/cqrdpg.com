...
app.get('/api/LazyBeewebsites', async (req, res) => {
    try {
        const result = await pool.request().query('SELECT id, name, url, notes FROM LazyBee.dbo.Websites ORDER BY id');
...
app.get('/api/getRecords', async (req, res) => {
    try {
        const recordsResult = await pool.request().query('SELECT * FROM Records');
...
app.get('/api/getMusicData', async (req, res) => {
    try {
        const musicResult = await pool.request().query('SELECT * FROM Music');
...
app.get('/api/getRealEstateData', async (req, res) => {
    try {
        const realEstateResult = await pool.request().query('SELECT * FROM RealEstate');
...
app.get('/api/getrealestatepicturecarouselimages', (req, res) => {
    ...
    fs.readdir(imageDir, (err, files) => {
        ...
        const imageUrls = imageFiles.map(file =>
            `https://www.cqrdpg.com/backend/images/Community/${encodeURIComponent(region)}/${encodeURIComponent(folder)}/${file}`
        );
...
{ //好友聊天//聊天👇
    ...
    app.post('/api/WeChatApp/messages/uploadImage',
        ...
        async (req, res) => {
            try {
                ...
                // 构建图片访问URL
                const imageUrl = `https://www.cqrdpg.com/backend/images/ChatImages/${sender_name}/${req.file.filename}`;
    ...
    app.get('/api/WeChatApp/getuserheadimage', (req, res) => {
        ...
        fs.readdir(imageDir, (err, files) => {
            ...
            // 返回第一张图片的URL
            const imageUrl = `https://www.cqrdpg.com/backend/images/ChatApp/${encodeURIComponent(username)}/headpicture/${imageFiles[0]}`;
            res.json({ imageUrl });
        });
    });
    ...
    app.post('/api/uploaduserheadimage', uploadheadimage.single('image'), (req, res) => {
        ...
        // 返回新的头像 URL
        const imageUrl = `https://www.cqrdpg.com/backend/images/ChatApp/${encodeURIComponent(username)}/headpicture/${req.file.filename}`;
        res.json({ imageUrl });
    });
    ...
    app.post('/api/themesettings/uploadchatbackground', uploadChatBackground.single('image'), (req, res) => {
        ...
        // 返回新的背景图片 URL
        const imageUrl = `https://www.cqrdpg.com/backend/images/ChatApp/${encodeURIComponent(username)}/chatbackgroundimage/${req.file.filename}`;
        res.json({ imageUrl });
    });
...
app.post('/api/savethemesettings', upload.none(), async (req, res) => {
    ...
    try {
        ...
        request.input('navbarFontColor', sql.NVarChar, navbarFontColor);
        request.input('navbarBackgroundColor', sql.NVarChar, navbarBackgroundColor);
        await request.query(query);
...
app.post('/api/update-room-state', async (req, res) => {
    ...
    try {
        ...
        const roomInfo = await pool.request()
            .input('room_name', sql.NVarChar, room_name)
            .query('SELECT * FROM ChatApp.dbo.MusicRooms WHERE room_name = @room_name');
...
app.post('/api/create-room', async (req, res) => {
    ...
    try {
        ...
        const checkResult = await pool.request()
            ...
            .query('SELECT COUNT(*) AS count FROM ChatApp.dbo.MusicRooms WHERE room_name = @room_name');
        ...
        await pool.request()
            ...
            .input('play_mode', sql.VarChar, play_mode)
            .query(`
...
app.post('/api/uploadmusic', uploadMusic.fields([
    ...
    try {
        ...
        const baseUrl = 'https://www.cqrdpg.com/backend/musics';

        // 使用全局连接池
        const result = await pool.request()
            .input('title', sql.NVarChar(255), title)
            .input('artist', sql.NVarChar(255), artist)
            .input('coverimage', sql.NVarChar(255), coverFile.filename)
...
app.post('/api/music-comments', async (req, res) => {
    ...
    try {
        const result = await pool.request()
            .input('music_id', sql.Int, Number(music_id))
            .input('music_title', sql.VarChar, music_title)
            .input('music_artist', sql.VarChar, music_artist)
            .input('user_name', sql.VarChar, user_name)
    ...
    try {
        const result = await pool.request()
            ...
            .query(`
                ...
                USING (VALUES (@user_name, @music_id, @music_title, @music_artist)) 
    ...
    try {
        ...
        const response = {
            ...
            images: imagesResult.recordset.map(img => ({
                ...img,
                url: `https://www.cqrdpg.com/backend/images/DressingGuidelinesData/${formattedDate}/${img.src}`
            }))
        };
...
app.get('/api/dressing-guidelines/today', async (req, res) => {
    try {
        ...
        const response = {
            ...
            images: imagesResult.recordset.map(img => ({
                ...img,
                url: `https://www.cqrdpg.com/backend/images/DressingGuidelinesData/${today}/${img.src}`
            }))
        };
        ...
        const resultsWithImages = await Promise.all(
            weatherResults.recordset.map(async (weatherItem) => {
                ...
                return {
                    ...
                    images: imagesResult.recordset.map(img => ({
                        ...img,
                        url: `https://www.cqrdpg.com/backend/images/DressingGuidelinesData/${weatherItem.date}/${img.src}`
                    }))
                };
            })
        );
    ...
    async (req, res) => {
        try {
            ...
            for (let i = 0; i < req.files.length; i++) {
                ...
                const imageUrl = `https://www.cqrdpg.com/backend/images/DressingGuidelinesData/${date}/${file.originalname}`;
            ...
            res.json({
                ...
                images: req.files.map(file => ({
                    ...
                    path: `https://www.cqrdpg.com/backend/images/DressingGuidelinesData/${date}/${file.originalname}`
                }))
            });
...
app.post('/projectapi/projects', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .input('project_source', sql.VarChar, project_source)
            .input('branch_or_sub_institution', sql.VarChar, branch_or_sub_institution)
            .input('commission_number', sql.VarChar, commission_number)
            .input('source_contact_name', sql.VarChar, source_contact_name)
            .input('source_contact_phone', sql.VarChar, source_contact_phone)
            .input('client_contact_name', sql.VarChar, client_contact_name)
            .input('client_contact_phone', sql.VarChar, client_contact_phone)
            .input('client_name', sql.VarChar, client_name)
            .input('project_name', sql.VarChar, project_name)
            .input('project_type', sql.VarChar, project_type)
            .input('evaluation_purpose', sql.VarChar, evaluation_purpose)
            .input('project_leader', sql.VarChar, project_leader)
            .input('draft_archive_date', sql.DateTime, draft_archive_date)
            .input('contract_signing_date', sql.DateTime, contract_signing_date)
            .input('project_progress', sql.VarChar, project_progress)
...
app.put('/projectapi/projects/:id', async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .input('project_progress', sql.VarChar, project_progress)
            .input('remarks', sql.Text, remarks)
            .input('assignment_time', sql.DateTime, assignment_time)
            .query(`
...
app.get('/api/getWardrobeStewardData', async (req, res) => {
    try {
        ...
        result.recordset.forEach(item => {
            // 构建两张图片URL
            const itemImageUrl = `https://www.cqrdpg.com/backend/images/WardrobeStewar/${item.category}/${item.item_code}.png`;
            const effectImageUrl = `https://www.cqrdpg.com/backend/images/WardrobeStewar/${item.category}/${item.item_code}effect.png`;
...
app.post('/api/createWordReport', async (req, res) => {
    ...
    try {
        const result = await pool.request()
            ...
            .input('floorNumber', sql.NVarChar(20), String(reportData.floorNumber || ''))
        ...
        try {
            ...
            res.json({
                ...
                images: newFiles.map(file => ({
                    filename: file.originalname,
                    path: `https://www.cqrdpg.com/backend/images/HousePricePictures/${reportsID}/${file.originalname}`
                }))
            });
...
app.get('/api/mucis/reactdemofavorites', async (req, res) => {
    try {
        ...
        if (search) {
            request = request.input('search', sql.VarChar, `%${search}%`);
        }
...
app.get('/api/reactdemorecommend', async (req, res) => {
    try {
        ...
        if (search) {
            countRequest = countRequest.input('search', sql.VarChar, `%${search}%`);
        }
        ...
        const dataQuery = `
            SELECT *,
                CASE 
                    ...
                    THEN CONCAT('https://www.cqrdpg.com/backend/musics/', coverimage)
                    ELSE 'https://www.cqrdpg.com/backend/musics/default.jpg'
                END as coverimage_url,
                CONCAT('https://www.cqrdpg.com/backend/musics/', src) as src_url
        ...
        if (search) {
            dataRequest = dataRequest.input('search', sql.VarChar, `%${search}%`);
        }
...
app.post('/api/Music/MusicRecentlyPlayed', async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            message: checkResult.recordset.length > 0 ? '更新记录成功' : '添加记录成功',
            action: checkResult.recordset.length > 0 ? 'updated' : 'added'
        });
    ...
    try {
      ...
      `);
        ...
        const insertResult = await pool.request()
            ...
            .input('coverimage', sql.NVarChar, 'https://www.cqrdpg.com/backend/musics/default.jpg')
...
app.post('/api/ReactDemomusiccomments', async (req, res) => {
    ...
    try {
        ...
        await pool.request()
            .input('music_id', sql.Int, parseInt(music_id))
            .input('music_title', sql.NVarChar, music_title)
            .input('music_artist', sql.NVarChar, music_artist)
            .input('user_name', sql.NVarChar, user_name)
        ...
        io.emit('new-comment', { music_id: parseInt(music_id) });
        res.json({ success: true });
    } catch (err) {
        console.error('Error submitting comment:', err);
        res.status(500).json({ error: 'Server error' });
            ...
            `;
                ...
                res.json({
                    ...
                    images: newFiles.map(file => ({
                        ...
                        buildingsPriceid: buildingsPriceid,
                        url: `https://www.cqrdpg.com/backend/images/BuildingsPricePictures/${buildingsPriceid}/${file.originalname}`
                    }))
                });
...
ORDER BY 
    ...
    app.post('/api/mergePdfs', async (req, res) => {
        try {
            ...
            const url = `https://www.cqrdpg.com/backend/mergedpdf/${filename}`;
...
7. 不要返回"权利其他状况"字段，将其拆解到对应的根级字段`;
    ...
    // OCR接口（兼容）
    app.post('/api/AIStudio/ocr', async (req, res) => {
        try {
            const { image } = req.body;
    ...
    // 健康检查
    app.get('/api/AIStudio/health', (req, res) => {
